...
async def websocket_client(query: str) -> Tuple[str, Sources]:
    ...
    uri = "ws://localhost:3001/?chatModel=GPT-4+omni+mini&chatModelProvider=openai&embeddingModel=Text+embedding+3+small&embeddingModelProvider=openai"
...
if __name__ == "__main__":
    with open('./formulate_query.json', 'r') as f:
        data = json.load(f)
        
    with open('perplexica_rag_knowledge.json') as f:
        perplexica_rag_knowledge = json.load(f)
        
    perplexica_rag_knowledge = {}

    for idx, key in tqdm(enumerate(list(data.keys()))):
        query = data[key]
        if key not in perplexica_rag_knowledge:
            gathered_messages, sources = asyncio.get_event_loop().run_until_complete(websocket_client(query))
            perplexica_rag_knowledge[key] = gathered_messages
            with open('perplexica_rag_knowledge.json', "w") as fout:
                json.dump(perplexica_rag_knowledge, fout, indent=2)