...
class IDBasedGroundingUIAgent:
    ...
    def predict(self, instruction: str, obs: Dict) -> List:
        ...
        if self.enable_reflection and self.turn_count > 0:
            print("inside self-reflection loop")
        ...
        if reflection:
            self.planning_agent.add_message('\nYou may use the reflection on the previous trajectory: ' + reflection +
                                            f"\nAccessibility Tree: {agent.linearized_accessibility_tree}.", image_content=obs['screenshot'])
        else:
            self.planning_agent.add_message(
        ...
        if agent.execution_feedback:
            self.planning_agent.add_message('\n The execution level feedback of the previous action is: ' + agent.execution_feedback, image_content=obs['screenshot'])
