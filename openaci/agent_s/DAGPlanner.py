...
file_path = os.path.join(working_dir, "kb", platform_name,"formulate_query.json")
...
class Planner:
    def __init__(
        ...
        self.plan_cache_path = os.path.join(working_dir, "kb", platform_name, "graph_agent_plans.json")
        self.dag_cache_path = os.path.join(working_dir, "kb", platform_name, "graph_agent_dags.json")
        self.search_query_cache_path = os.path.join(
            working_dir, "kb", platform_name, "formulate_query.json"
        )
    ...
    def retrieve_knowledge(self, instruction, current_state, engine):
        ...
        try:
            query_path = os.path.join(working_dir, "kb", platform_name, "formulate_query.json")
        ...
        if instruction in formulate_query.keys() and formulate_query[instruction]:
            ...
            self.rag_agent.add_message(
               f"To use google search to get some useful information, first carefully analyze the accessibility tree of the current desktop UI state, then given the task instruction, formulate a question that can be used to search on the Internet for information in helping with the task execution.\nThe question should not be too general or too specific, but it should be based on the current desktop UI state (e.g., already open website or application). You should expect the google search will return you something useful based on the question. Since it is a desktop computer task, make sure to mention the corresponding task domain in the question and also mention the {platform_name} OS if you think the OS matters. Please ONLY provide the question.\nQuestion:"
            )
        ...
        if engine == "llm":
            ...
            file = os.path.join(working_dir, "kb", platform_name, "perplexica_rag_knowledge.json")
    ...
    def retrieve_lifelong_learning_reflection(self, instruction):
        ...
        try:
            lifelong_learning_reflection_dicts = json.load(
                open(
                    os.path.join(
                        working_dir, "kb", platform_name, "lifelong_learning_knowledge_base.json"
                    )
                )
            )
    ...
    def retrieve_most_similar_knowledge(self, instruction):
        ...
        try:
            ...
            try:
                with open(os.path.join(working_dir, "kb", platform_name, "embeddings.pkl"), "rb") as f:
            ...
            with open(os.path.join(working_dir, "kb", platform_name, "embeddings.pkl"), "wb") as f:
    ...
    def _generate_step_by_step_plan(
        ...
        generator_message = (
            ...
            f"The current open applications are {agent.get_current_applications()}"
            + (
